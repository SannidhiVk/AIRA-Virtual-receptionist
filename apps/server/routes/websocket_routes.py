                ...
                while True:
                    try:
                        ...
                        if "disconnect" in str(exc).lower():
                            logger.info("[LISTENER] Client disconnected (RuntimeError).")
                            break
                        raise
                    ...
                    if current_mode == "PASSIVE" and previous_mode != "PASSIVE":
                        logger.info("[LISTENER] 🧹 Transitioned to PASSIVE. Wiping buffers and OWW memory.")
                        oww_carry.clear()
                        vad_carry.clear()
                        audio_buffer.clear()
                        try:
                    ...
                    if raw_bytes is not None and len(raw_bytes) > 0:
                        ...
                        if session_state["mode"] == "PASSIVE":
                            ...
                            while len(oww_carry) >= OWW_CHUNK_BYTES:
                                ...
                                if triggered:
                                    ...
                                    silence_frames = 0
                                    speech_seen    = False
                                    logger.info(f"[OWW] 🔔 TRIGGERED at score={score:.4f} — switching to ACTIVE")
                        ...
                        # ── ACTIVE: VAD + accumulate ───────────────────────────
                            ...
                            if speech_seen and silence_frames >= SILENCE_FRAME_THRESHOLD:
                                ...
                                logger.info(f"[VAD] End of utterance — buffer={len(audio_buffer)} bytes ({dur_s:.2f}s)")
        ...
        async def brain():
            try:
                while True:
                    ...
                    audio, word_timings = (
                        await tts_processor.synthesize_remaining_speech_with_timing(reply_text)
                    )
                    ...
                    if audio is not None and len(audio) > 0:
                        ...
                        logger.info(f"[BRAIN] Speaking for {audio_duration:.2f}s. Pausing listening.")
                    ...
                    if session_state["mode"] == "SPEAKING":
                        ...
                        session_state["mode"] = "PASSIVE"
                        await websocket.send_text(json.dumps({"state": "passive"}))
                        logger.info("[BRAIN] Playback complete. Back to PASSIVE (listening for wake word).")
