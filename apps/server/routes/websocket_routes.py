...
async def websocket_endpoint(websocket: WebSocket, client_id: str):
    ...
    try:
        await websocket.send_text(json.dumps({'status':'connected','client_id':client_id,'state':'passive'}))
        ...
        async def listener():
            ...
            while True:
                ...
                if current_mode == 'PASSIVE' and previous_mode != 'PASSIVE':
                    oww_carry.clear(); audio_buffer.clear(); speech_seen = False
                    try:
                        ww_service.model.reset()
                    except Exception:
                        pass
                elif current_mode == 'FOLLOWUP' and previous_mode != 'FOLLOWUP':
                    audio_buffer.clear(); speech_seen = False; followup_entered_at = time.time()
                    bytes_received_count = 0 
                    logger.info(f"[{client_id}] Entered FOLLOWUP/LISTENING mode. Waiting for audio...")
                ...
                if bytes_received_count == 1:
                    logger.info(f"[{client_id}] Successfully receiving audio stream from frontend (Mode: {current_mode})")
                ...
                if session_state['mode'] == 'PASSIVE':
                    ...
                    while len(oww_carry) >= OWW_CHUNK_BYTES:
                        ...
                        if triggered:
                            logger.info(f"[{client_id}] Wake word triggered! Score: {score}")
                    ...
                    if current_mode == 'FOLLOWUP' and not speech_seen and (time.time() - followup_entered_at > FOLLOWUP_TIMEOUT_SECONDS):
                        logger.info(f"[{client_id}] Followup timeout reached (no speech detected). Returning to PASSIVE.")
                        session_state['mode'] = 'PASSIVE'
                        await websocket.send_text(json.dumps({'state':'passive'}))
                        continue
                    ...
                    if len(audio_np) >= SILERO_WINDOW_SAMPLES:
                        ...
                        if speech:
                            ...
                            current_silence_ms = ((total_samples - last_speech_end) / SAMPLE_RATE) * 1000
                            
                            if current_silence_ms >= MAX_SILENCE_MS:
                                logger.info(f"[{client_id}] VAD detected speech end (User paused for {current_silence_ms:.0f}ms). Processing...")
                                session_state['mode'] = 'PROCESSING'
                                await websocket.send_text(json.dumps({'state':'processing'}))
                                wav_bytes = create_wav_from_pcm(bytes(audio_buffer))
                                text = await whisper_processor.transcribe_audio(wav_bytes)
                                
                                logger.info(f"[{client_id}] Whisper transcribed: '{text}'")
                                ...
                                if text and text not in ('NOISE_DETECTED','NO_SPEECH'):
                                    ...
                                    session_state['mode'] = 'PASSIVE'
                                    await websocket.send_text(json.dumps({'state':'passive'}))
                                audio_buffer.clear(); speech_seen = False
        ...
        async def brain():
            while True:
                ...
                if audio is not None and len(audio) > 0:
                    ...
                    await websocket.send_text(json.dumps({'audio':b64,'word_timings':word_timings,'sample_rate':24000,'method':'native_kokoro_timing','state':'speaking'}))
        ...
        tasks =[asyncio.create_task(listener()), asyncio.create_task(brain()), asyncio.create_task(send_keepalive())]
