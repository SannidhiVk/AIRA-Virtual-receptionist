...
def clear_session_state(client_id: str, retain_name=False) -> None:
    ...
    if retain_name and old_state.get("visitor_name"):
        ...
        try: GroqProcessor.get_instance().reset_history(client_id)
...
def _determine_visitor_type(text: str, purpose: str, current_type: str) -> str:
    ...
    if re.search(r'\b(amazon|flipkart|delivery|courier|package)\b', combined): return "Delivery"
    if re.search(r'\b(vendor|electrician|plumber|maintenance)\b', combined): return "Contractor/Vendor"
...
def _lookup_employee(search_term: str) -> Optional[Any]:
    ...
    db = SessionLocal()
    try:
        emp = db.query(Employee).filter(or_(Employee.name.ilike(f"%{clean}%"), Employee.role.ilike(f"%{clean}%"), Employee.department.ilike(f"%{clean}%"))).first()
        if emp: return emp
        if "manager" in clean:
            dept = clean.replace("manager", "").strip()
            return db.query(Employee).filter(and_(Employee.department.ilike(f"%{dept}%"), Employee.role.ilike(f"%manager%"))).first() if dept else None
        return None
    finally: db.close()
...
def _commit_checkin(state: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        emp = _lookup_employee(state["meeting_with_raw"]) if state.get("meeting_with_raw") else None

        log = ReceptionLog(
            visitor_id=visitor.id, employee_id=emp.id if emp else None,
            person_type=state["visitor_type"], check_in_time=datetime.utcnow(),
            purpose=state["purpose"], notes=f"Meeting with: {state.get('meeting_with_raw')}"
        )
        db.add(log)
        db.commit()
        ...
        if emp:
            from services.notify_teams import send_teams_arrival
            send_teams_arrival(emp.name, state.get("visitor_name", "Unknown"), state["visitor_type"], state["purpose"])
        return True
...
def _commit_meeting(emp_name: str, emp_email: Optional[str], date_str: str, time_str: str, purpose: str, org_name: str, org_email: str) -> bool:
    from receptionist.database import schedule_meeting
    res = schedule_meeting(organizer_name=org_name, organizer_type="visitor", employee_name=emp_name, meeting_date=date_str, meeting_time=time_str, purpose=purpose)
    
    if res:
        from services.targeted_scheduler import schedule_meeting_targeted
        schedule_meeting_targeted(emp_email, emp_name, org_email, org_name, date_str, time_str, purpose)
    return res is not None

def _clean_entity(val: Any) -> Optional[str]:
    s = str(val).strip() if val else ""
    return s if s and s.lower() not in ("null", "none", "") and s.lower() not in PRONOUNS else None

def _merge_checkin_entities(state: Dict[str, Any], entities: Dict[str, Any], client_id: str, user_query: str) -> None:
    name, email = _clean_entity(entities.get("visitor_name")), _clean_entity(entities.get("email"))
    if name: state["visitor_name"] = name.capitalize() # Overrides and trusts user
    if email: state["visitor_email"] = email
    ...
    if purpose and len(purpose) > 3: state["purpose"] = purpose.capitalize()

    state["visitor_type"] = _determine_visitor_type(user_query, str(purpose or ""), _clean_entity(entities.get("visitor_type")))
    ...
    if str(entities.get("employee_name") or "").strip().lower() in PRONOUNS and client_id:
        ...
        if resolved and not state["meeting_with_raw"]: state["meeting_with_raw"] = resolved
...
async def _handle_scheduling(client_id: str, user_query: str, state: Dict[str, Any], entities: Dict[str, Any], intent: str) -> str:
    ...
    if not state.get("visitor_name"): return await _llm_reply("Ask the visitor for their name.", {}, user_query, client_id=client_id)
    if not state["sched_employee_raw"]: return await _llm_reply("Ask who they would like to schedule the meeting with.", {"visitor_name": state.get("visitor_name")}, user_query, client_id=client_id)
    ...
    if not state["sched_employee_name"]:
        ...
        if not emp:
            ...
            return await _llm_reply(f"Tell the visitor '{bad}' was not found. Clarify who they want.", {"visitor_name": state.get("visitor_name")}, user_query, client_id=client_id)
    ...
    if not state["sched_date"]: return await _llm_reply(f"Ask what date to schedule with {emp_display}.", {"visitor_name": state.get("visitor_name")}, user_query, client_id=client_id)
    if not state["sched_time"]: return await _llm_reply("Ask what time to schedule.", {"visitor_name": state.get("visitor_name")}, user_query, client_id=client_id)
    if not state["sched_purpose"]: return await _llm_reply("Ask the purpose of the meeting.", {"visitor_name": state.get("visitor_name")}, user_query)

    if not state["sched_pending_confirm"]:
        state["sched_pending_confirm"] = True
        return await _llm_reply("Read back the meeting details and ask if you should confirm.", {"visitor_name": state.get("visitor_name"), "employee_name": emp_display, "date": state["sched_date"], "time": state["sched_time"], "purpose": state["sched_purpose"]}, user_query, client_id=client_id)

    is_yes = intent == "confirm" or any(w in user_query.lower().split() for w in ["yes", "yeah", "yep", "sure", "ok", "okay", "please"])
    is_no = intent == "cancel" or any(w in user_query.lower().split() for w in ["no", "cancel", "nevermind"])

    if is_no:
        state["scheduling_active"] = False
        return await _llm_reply("Tell them you cancelled the request.", {"visitor_name": state.get("visitor_name")}, user_query, client_id=client_id)
    ...
    if is_yes:
        ...
        if success:
            ...
            return await _llm_reply("Confirm the meeting is scheduled.", {"visitor_name": state.get("visitor_name"), "employee_name": emp_display}, user_query, client_id=client_id)
        return await _llm_reply("Apologize for a technical issue saving the meeting.", {"visitor_name": state.get("visitor_name")}, user_query, client_id=client_id)

    return await _llm_reply("Ask clearly if you should log the meeting.", {"visitor_name": state.get("visitor_name")}, user_query, client_id=client_id)
...
async def route_query(client_id: str, user_query: str) -> str:
    ...
    is_greeting = re.search(r"\b(hello|hi|hey|good morning|good afternoon)\b", user_query.lower())
    is_goodbye = re.search(r"\b(bye|goodbye|thank you|thanks|no thanks)\b", user_query.lower())

    active_states = [State.COLLECTING_NAME, State.COLLECTING_HOST, State.COLLECTING_PURPOSE]
    if is_greeting and state["conv_state"] not in active_states and not state.get("scheduling_active"):
        clear_session_state(client_id, retain_name=False)
        state = get_session_state(client_id)
    ...
    if is_goodbye and state["conv_state"] in [State.COMPLETED, State.INIT]:
        clear_session_state(client_id, retain_name=False)
        return await _llm_reply("Warmly say goodbye and wish them a great day. DO NOT ask if they need help.", {}, user_query, client_id)
    ...
    if intent == "facility_request": return await _llm_reply("Assure them you are pinging administration.", {"visitor_name": state.get("visitor_name")}, user_query, client_id=client_id)
    ...
    if intent == "employee_lookup":
        target = _clean_entity(entities.get("employee_name")) or _clean_entity(entities.get("role")) or state.get("meeting_with_resolved")
        ...
        return await _llm_reply(f"Apologize warmly that you couldn't find '{target}'.", {"visitor_name": state.get("visitor_name")}, user_query, client_id=client_id)
    ...
    if intent == "general_conversation" and state["conv_state"] != State.COMPLETED:
        return await llm.get_response(client_id, user_query, {"company_name": COMPANY_NAME, "visitor_name": state.get("visitor_name") or "Visitor"})

    emp = _lookup_employee(state.get("meeting_with_resolved") or state.get("meeting_with_raw"))
    info = {"company_name": COMPANY_NAME, "visitor_name": state.get("visitor_name") or "Visitor", "status": "Visitor is checked in and waiting."}
    if emp: info["dynamic_employee"] = f"Name: {emp.name} | Role: {emp.role} | Location: {getattr(emp, 'location', 'N/A')}"
...
async def _advance_checkin(state: Dict[str, Any], user_query: str, client_id: str) -> str:
    ...
    if state.get("is_delivery") and not state.get("meeting_with_raw"):
        ...
        return await _llm_reply("Ask the delivery person who the package is for.", {}, user_query, client_id=client_id)

    if not state.get("visitor_name"):
        state["conv_state"] = State.COLLECTING_NAME
        return await _llm_reply(f"Welcome the visitor. Ask for their name.", {}, user_query, client_id=client_id)
    ...
    if not state.get("purpose"):
        ...
        return await _llm_reply(f"Ask the purpose of their visit with {emp_display}.", {"visitor_name": state["visitor_name"], "employee_name": emp_display}, user_query, client_id=client_id)
...
async def _complete_checkin(state: Dict[str, Any], user_query: str, client_id: str) -> str:
    ...
    if state.get("is_delivery"):
        ...
        return await _llm_reply(f"Thank the delivery person. Tell them to leave the package at the front desk, you will ping {target}.", {"visitor_name": state["visitor_name"], "employee_name": target}, user_query, client_id=client_id)

    if emp:
        return await _llm_reply(f"Tell the visitor they are checked in. You are notifying {emp.name}. Direct them to {getattr(emp, 'location', 'their workspace')}.", {"visitor_name": state["visitor_name"], "employee_name": emp.name}, user_query, client_id=client_id)
    
    return await _llm_reply(f"Tell the visitor they are checked in. You are notifying the {state['meeting_with_raw']} team.", {"visitor_name": state["visitor_name"], "employee_name": state['meeting_with_raw']}, user_query, client_id=client_id)