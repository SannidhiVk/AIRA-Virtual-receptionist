...
def _send_slack_notification_thread(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"❌ Failed to post to Slack. Status: {response.status_code}, Body: {response.text}")
...
def send_slack_arrival(
    ...
    with _notify_lock:
        ...
        if _last_notified.get(session_id) == visitor_name:
            logger.warning(f"Blocking duplicate notification for {visitor_name} in session {session_id}")
            return
