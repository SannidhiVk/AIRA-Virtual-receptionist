...
def checkout_visitor(badge_id: str) -> None:
    ...
    try:
        log_entry = (
            ...
            .filter(
                ReceptionLog.badge_id == badge_id, ReceptionLog.check_out_time.is_(None)
            )
            .first()
        )
...
def set_company_details(
    ...
    if address:
        set_setting("company_address", address)
    if phone:
        set_setting("company_phone", phone)
    if email:
        set_setting("company_email", email)
    if website:
        set_setting("company_website", website)
...
def get_hr(name: str = "HR") -> Optional[Employee]:
    ...
    try:
        ...
        if not emp:
            emp = (
                session.query(Employee)
                .filter(Employee.department.ilike("hr"), Employee.is_public == True)
                .first()
            )
        return emp
    except Exception as exc:
...
def schedule_meeting(
    ...
    try:
        ...
        if not emp:
            logger.warning(
                "Employee '%s' not found for meeting schedule.", employee_name
            )
            return None
